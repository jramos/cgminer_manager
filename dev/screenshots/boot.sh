...
for port in "$FAKE_MONITOR_PORT" "$MANAGER_PORT"; do
...
start_process manager env \
  ...
  CGMINER_MANAGER_FAKE_PING='{"timestamp":1776416646,"available_miners":6,"unavailable_miners":0}' \
  PORT="$MANAGER_PORT" \
  BIND="127.0.0.1" \
...
Logs:   dev/screenshots/.run/{fake_monitor,manager}.log
