...
`CgminerCommander#fan_out_query` / `#fan_out_write`, `PoolManager#run_each`, and `ViewModels.fetch_snapshots_for` all delegate to `CgminerManager::ThreadedFanOut.map(items, thread_cap:) { ... }`. The helper pre-loads a `Queue` with `[index, item]` pairs, spawns `min(thread_cap, items.size)` worker threads, each popping with `queue.pop(true)` and writing to a shared `results` array under a `Mutex`. When the queue is drained, `queue.pop(true)` raises `ThreadError` and the worker breaks out of its loop. Callers supply the block that does site-specific error capture and entry wrapping; the helper returns an ordered `Array` matching input order.

`ViewModels.fetch_snapshots_for` re-keys the ordered array into a `miner_id → tile` hash after the helper returns, preserving the shape the dashboard template expects.
