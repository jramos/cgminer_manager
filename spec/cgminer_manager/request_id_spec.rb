...
describe CgminerManager::RequestId do
  let(:inner_app) { ->(env) { [200, {}, [env['cgminer_manager.request_id']]] } }
  let(:middleware) { described_class.new(inner_app) }

  context 'when the request carries X-Cgminer-Request-Id' do
    it "propagates the inbound value to env['cgminer_manager.request_id']" do
